...
class Japur_Source_Sync {
    ...
    private static function fetch_xml($url) {
        $r=wp_safe_remote_get($url,['timeout'=>20,'redirection'=>4,'user-agent'=>'JaPurSourceSync/1.3']);
    ...
    private static function sitemap_is_post($url) {
        ...
        if(preg_match('~(^|[-_])(page|category|categories|tag|tags|author|attachment|media|image|images|product|products|taxonomy|taxonomies|post-format|users?)([-_]|\.|$)~i',$base)) return false;
        // Banyak publisher besar memakai sitemap_web.xml / sitemap_news.xml untuk artikel.
        if(preg_match('~(^|[-_])(post|posts|news|web)([-_]|\.|$)~i',$base)) return true;
        return false;
    }
...
<style>#jss-source-sync{display:none}.jss-source-panel{margin-top:6px}.jss-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.jss-grid label{display:block;margin-bottom:3px;font-size:12px}.jss-grid input{width:100%;min-height:34px}.jss-source-toolbar{display:flex;justify-content:space-between;align-items:center;gap:7px;margin:0 0 6px}.jss-filter{min-width:105px;height:32px}.jss-source-actions{display:flex;gap:6px;flex-wrap:wrap;margin:7px 0}.jss-source-actions .button{min-height:32px;padding:0 9px}.jss-source-list{border-top:1px solid #eee;margin-top:6px}.jss-source{display:flex;align-items:center;gap:6px;padding:5px 0;border-bottom:1px solid #eee;font-size:12px}.jss-source strong{flex:0 0 auto}.jss-source small{color:#646970;word-break:break-all;flex:1;min-width:0}.jss-source button{margin-left:0;padding:0 5px}.jss-qhead{display:flex;justify-content:space-between;align-items:center;margin-top:8px;padding:7px 0;border-top:1px solid #eee;font-size:13px}.jss-item{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:7px;padding:6px 7px;border:1px solid #eee;border-radius:7px;margin:4px 0}.jss-item-title{font-weight:600;font-size:12px;line-height:1.25;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;overflow:hidden}.jss-item-url{font-size:10px;line-height:1.25;color:#646970;word-break:break-all;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin-top:2px}.jss-use{white-space:nowrap!important;font-size:11px!important;line-height:28px!important;height:28px!important;min-height:28px!important;padding:0 9px!important}.jss-empty{color:#646970;padding:7px 0;font-size:12px}.jss-count{font-size:11px;color:#646970}.jss-source-note{margin-top:6px;color:#646970;font-size:10px;line-height:1.35}@media(max-width:700px){.jss-grid{grid-template-columns:1fr;gap:5px}.jss-source-actions{margin:6px 0}.jss-source{padding:4px 0}.jss-source small{display:none}.jss-source button{font-size:11px}.jss-item{grid-template-columns:minmax(0,1fr) auto;gap:5px;padding:5px 6px;margin:3px 0}.jss-item-title{-webkit-line-clamp:1}.jss-item-url{-webkit-line-clamp:1;font-size:9px}.jss-use{width:auto!important}.jss-source-toolbar{align-items:center}.jss-filter{min-width:100px}}</style>
<div id="jss-source-sync"><div class="jss-source-panel"><div class="jss-grid"><div><label>Nama Website</label><input id="jss-name" type="text" placeholder="Contoh: Sumber Berita"></div><div><label>Domain Website</label><input id="jss-url" type="url" placeholder="https://contoh.com"></div></div><div class="jss-source-actions"><button type="button" class="button button-primary" id="jss-add">Tambah Sumber</button><button type="button" class="button" id="jss-scan-all">Cek Artikel Terbaru</button></div><div id="jss-sources" class="jss-source-list"></div><div class="jss-qhead"><strong>Artikel Terbaru Terdeteksi</strong><span><span id="jss-count">0</span> artikel</span></div><div class="jss-source-toolbar"><span class="jss-count" id="jss-range-label">Menampilkan 10 terbaru</span><select id="jss-limit" class="jss-filter"><option value="10">10 terbaru</option><option value="25">25 terbaru</option><option value="50">50 terbaru</option><option value="all">Semua</option></select></div><div id="jss-list"></div><div class="jss-source-note">Hanya URL dari sitemap post/news/web yang diproses. Setelah Extract berhasil, artikel otomatis dikeluarkan dari daftar tetapi tetap ditandai sebagai sudah digunakan.</div></div></div>
<script>jQuery(function($){var box=$('#jss-source-sync');if(!box.length)return;var sourceCard=$('#jaf-source-card');if(!sourceCard.length)return;var tabs=sourceCard.find('.jwp-source-tabs').first();var urlPanel=$('#jaf-source-url');var manualPanel=$('#jaf-source-manual');if(!tabs.length||!urlPanel.length||!manualPanel.length)return;var tab=$('<button type="button" class="button jwp-source-tab" data-source="web">Web Sumber</button>');tabs.prepend(tab);var panel=$('<div id="jaf-source-web" class="jwp-source-panel" style="display:none"></div>');panel.append(box.children('.jss-source-panel'));manualPanel.before(panel);box.remove();var allQueue=<?php echo wp_json_encode(array_values($q));?>;function req(action,data,done){data=data||{};data.action=action;data.nonce=(window.JAF&&JAF.nonce)||'';$.post((window.JAF&&JAF.ajax)||ajaxurl,data,done)}function esc(t){return $('<div>').text(t||'').html()}function setTab(mode){sourceCard.find('.jwp-source-tab').removeClass('active');sourceCard.find('.jwp-source-panel').hide();sourceCard.find('.jwp-source-tab[data-source="'+mode+'"]').addClass('active');if(mode==='web')$('#jaf-source-web').show();else $('#jaf-source-'+mode).show()}tabs.on('click','.jwp-source-tab',function(){setTab($(this).data('source'))});function renderSources(items){var h='';$.each(items,function(_,s){h+='<div class="jss-source"><strong>'+esc(s.name)+'</strong><small>'+esc(s.url)+'</small><button class="button jss-scan" data-id="'+esc(s.id)+'">Cek</button><button class="button-link-delete jss-del" data-id="'+esc(s.id)+'">Hapus</button></div>'});$('#jss-sources').html(h||'<div class="jss-empty">Belum ada website sumber.</div>')}function renderQueue(items){allQueue=Array.isArray(items)?items:[];$('#jss-count').text(allQueue.length);var limit=$('#jss-limit').val()||'10',shown=limit==='all'?allQueue:allQueue.slice(0,Number(limit));$('#jss-range-label').text('Menampilkan '+shown.length+' '+(limit==='all'?'artikel':'terbaru'));var h='';$.each(shown,function(_,x){h+='<div class="jss-item" data-id="'+esc(x.id)+'"><div><div class="jss-item-title">'+esc(x.title||'Artikel sumber')+'</div><div class="jss-item-url">'+esc(x.url)+(x.date?' • '+esc(x.date):'')+'</div></div><button type="button" class="button button-primary jss-use" data-id="'+esc(x.id)+'" data-url="'+esc(x.url)+'">Gunakan</button></div>'});$('#jss-list').html(h||'<div class="jss-empty">Belum ada artikel baru.</div>')}function load(){req('jss_get_queue',{},function(r){if(!r.success)return;renderSources(r.data.sources||[]);renderQueue(r.data.queue||[])})}$('#jss-limit').on('change',function(){renderQueue(allQueue)});$('#jss-add').on('click',function(){var b=$(this);b.prop('disabled',true);req('jss_save_source',{name:$('#jss-name').val(),url:$('#jss-url').val(),enabled:1},function(r){b.prop('disabled',false);if(!r.success){alert(r.data.message);return}$('#jss-name,#jss-url').val('');load()})});$('#jss-scan-all').on('click',function(){var b=$(this);b.prop('disabled',true);req('jss_get_queue',{},function(r){var sources=(r.success&&r.data.sources)||[],i=0;function next(){if(i>=sources.length){b.prop('disabled',false);load();return}var id=sources[i++].id;req('jss_scan_source',{id:id},function(){next()})}next()})});$(document).on('click','.jss-scan',function(){var id=$(this).data('id'),b=$(this);b.prop('disabled',true);req('jss_scan_source',{id:id},function(r){b.prop('disabled',false);if(!r.success)alert(r.data.message);load()})});$(document).on('click','.jss-del',function(){if(!confirm('Hapus website sumber ini?'))return;req('jss_delete_source',{id:$(this).data('id')},function(){load()})});$(document).on('click','.jss-use',function(){var id=$(this).data('id'),url=$(this).data('url'),b=$(this);b.prop('disabled',true).text('Mengambil...');req('jaf_extract_url',{url:url},function(r){if(!r.success){b.prop('disabled',false).text('Gunakan');alert(r.data&&r.data.message?r.data.message:'Gagal mengambil artikel.');return}var m=r.data.material||r.data.content||'';if($('#jaf-material').length)$('#jaf-material').val(m).trigger('input').trigger('change');allQueue=$.grep(allQueue,function(x){return x.id!==id});renderQueue(allQueue);req('jss_consume_queue',{id:id},function(){load()});$('html,body').animate({scrollTop:$('#jaf-material').offset().top-100},300)})});load()});</script>
<?php }
}
